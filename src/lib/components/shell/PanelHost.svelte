<script lang="ts">
  ...
  import PanelHandle from "$lib/ui/PanelHandle.svelte";
  ...
  let aspect = $state<"compact" | "portrait" | "landscape" | "square">("portrait");
  ...
  const addProduct2 = (p: Product, e: HTMLElement | null) => (onAddProduct ?? noopProduct2)(p, e);
  ...
  const hasOwnHeader = $derived(["conversation", "checkout", "address-select", "address-form", "wishlist", "delivery-info", "order-tracking", "lists", "sessions", "orders", "address-book", "memories"].includes(panel.type));
...
<div class="tile-host" bind:this={hostEl} role="region" aria-label={panel.title}>
  <PanelHandle panelId={panel.id} title={panel.title} canMinimize={ui.isSplit} />
  <div class="host-body">
...
<style>
  ...
  .host-body { flex: 1; min-height: 0; overflow: hidden; display: flex; flex-direction: column; }

  @media (prefers-reduced-motion: reduce) {
    .tile-host { animation: none; }
  }
</style>
