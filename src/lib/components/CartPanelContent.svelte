...
<div class="flex h-full min-h-0 flex-col">
    <PanelHeader title="Cart" icon={ShoppingBag} count={items.length}>
        {#snippet actions()}
            {#if items.length > 0}
                <PanelActionButton
                    label="Create order"
                ...
                {#each items as item (item.product.id)}
                    <div class="flex items-start gap-3 p-3">
                        {#if item.product.imageUrl && !imgErrors[item.product.id]}
                            ...
                            />
                        {:else}
                            <div class="flex h-14 w-14 items-center justify-center rounded-lg bg-[var(--color-muted)]">
                                <ShoppingBag class="h-5 w-5 text-[var(--color-muted-foreground)]/40" />
                            </div>
                        {/if}
                        <div class="min-w-0 flex-1">
                            <h4 class="line-clamp-2 text-xs font-medium leading-tight text-[var(--color-foreground)]">
                                {item.product.name}
                            </h4>
                            {#if item.product.price == null}
                                <p class="mt-0.5 text-xs font-semibold text-[var(--color-muted-foreground)]">Price on request</p>
                            {:else}
                                <p class="mt-0.5 text-sm font-bold text-[var(--color-foreground)]">
                                    {formatMoney(item.product.price, item.product.currency)}
                                </p>
                            {/if}
                            <div class="mt-1.5 flex items-center gap-1.5">
                                <Button
                                    onclick={() => onQuantity?.(item.product.id, item.quantity - 1)}
                                    type="button"
                                    variant="outline"
