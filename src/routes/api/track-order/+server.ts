...
import { normalizeOrderNumber, normalizeTracking } from "$lib/shopping-engine";
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (!orderNumber.ok) return Response.json({ error: orderNumber.error.message }, { status: 400 });


    const raw = await callMcpTool("kapruka_track_order", { order_number: orderNumber.data });
